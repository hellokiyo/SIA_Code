...
// 더하기 함수상자 만들기 : a, b라는 2개의 위쪽 구멍을 만들어둠
...
const result = add(a, b); // add 함수 실행
console.log(`더하기 결과 ${result}`); // 결과 출력
...
// add5 호출
add5(10,10,(err, result) => { //보통 구멍을 두개 만들어서 err를 넣음
    if(err) {
...
// setTimeout을 사용한 비동기 함수
const add6 = (a,b,callback) => {
    setTimeout( ()=> { //한번만 실행 (0.5초 후 실행)
        const result = a + b;
...
divide6(200,10, (err, result) => {
    ...
    console.log(`divide6안에서 나누기 결과 : ${result}`);
})

// add6 실행
add6(10,10,(err, result) => { //보통 구멍을 두개 만들어서 err를 넣음
    if(err) {
        console.error(`에러 발생 -> ${err}`);
        return;
    }
...
const doCalc = async() => { // async 함수 정의
    try {
        ...
        const result1 = await divide7(200,0); // 0으로 나눠서 에러 발생
